...
def get_config(config_file=os.path.expanduser('~/.config/weltschmerz/weltschmerz.cfg')):
    ...
    parser.add_argument('-S', '--screenshot-files', nargs='*', help='screenshot file to tag',
                        default=None, dest='screenshot_files')
...
def parse_screenshot(screenshot: str) -> anime.TitleScreenShot:
    try:
        ...
        tss = anime.TitleScreenShot(
            ...
            source_file_name = try_parse_xmp(metadata, 'Xmp.anime.AnimeFileName') or anime_filename_from_filename(screenshot),
            ...
            source_file_duration_raw = try_parse_xmp(metadata, 'Xmp.anime.AnimeFileDurationRaw'),

            time_position = metadata['Xmp.anime.AnimeFileTime'].value,
            time_position_raw = try_parse_xmp(metadata, 'Xmp.anime.AnimeFileTimeRaw') or raw_timepos_from_filename(screenshot),
            title_type = try_parse_xmp(metadata, 'Xmp.anime.ScreenshotType'),
        )
        try:
            anidb_file = dbs.session.query(anime.File).filter(anime.File.hash_ed2k==metadata['Xmp.anime.AnimeFileEd2k'].value).one()
