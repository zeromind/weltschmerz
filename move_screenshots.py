...
if __name__ == "__main__":
    ...
    screenshots = (
        ...
        .filter(
            anime.TitleScreenShot.filename.like(f'{config.source_basedir.rstrip("/")}%')
        )
        .all()
    )
    ...
    for screenshot in screenshots:
        try:
            known_file = (
                ...
                .filter(anime.File.hash_ed2k == screenshot.source_file_hash_ed2k)
                .one()
            )
            aid = known_file.aid
        except:
        ...
        if os.path.dirname(screenshot.filename) not in [target_dir]:
            if os.path.isfile(screenshot.filename):
                print(
                    f'file not sorted yet "{screenshot.filename}" is in "{os.path.dirname(screenshot.filename)}", should be "{target_dir}"'
                )
                if not os.path.isfile(
                    os.path.join(target_dir, os.path.basename(screenshot.filename))
                ):
                    pathlib.Path(target_dir).mkdir(parents=True, exist_ok=True)
                    print(
                        f'##### INFO: moving "{screenshot.filename}" to "{target_dir}"'
                    )
                    if not config.dry_run:
                        shutil.move(screenshot.filename, target_dir)
                        screenshot.filename = os.path.join(
                            target_dir, os.path.basename(screenshot.filename)
                        )
                        ...
                        print(
                            f"##### INFO: dry-run: source {screenshot.filename} target {target_dir}"
                        )
                        print(
                            f"##### INFO: dry-run: {os.path.join(target_dir, os.path.basename(screenshot.filename))}"
                        )

                else:
                    print(
                        f'##### WARNING: "{os.path.join(target_dir, os.path.dirname(screenshot.filename))}" already present'
                    )
