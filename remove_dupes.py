...
def get_config(config_file: str = 'weltschmerz.cfg'):
    ...
    parser = argparse.ArgumentParser(
        description='Remove dupes from db and disk.')
...
def update_mtimes(files):
    ...
    for file in available_files:
        if os.path.getmtime(file.full_path) != oldest_mtime:
            print(f'setting atime/mtime to {datetime.datetime.fromtimestamp(oldest_mtime).isoformat()} for {file.full_path}')
...
if __name__ == '__main__':
    ...
    for i, (hash_ed2k, filesize) in enumerate(known_dupes, start=1):
        ...
        if len(file_names) == 1:
            ...
            print('File to keep:')
            # continue
            selection = pyselect.select(available_files, 'f"{option.filename} ({option.directory})"')
        # keep only files to remove in array
        available_files.remove(selection)
        print([file.full_path for file in available_files])
        for file in available_files:
            if os.path.isfile(selection.full_path) and os.path.realpath(selection.full_path) != os.path.realpath(file.full_path):
