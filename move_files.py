...
def get_config(config_file:str = 'weltschmerz.cfg'):
    ...
    parser.add_argument('--target-crc32', '-C', help='add crc32 to target filename',
                        default=False, dest='target_crc32', action='store_true')
...
if __name__ == '__main__':
    ...
    known_files = dbs.session.query(anime.LocalFile).join(anime.File, anime.LocalFile.hash_ed2k==anime.File.hash_ed2k).filter(anime.LocalFile.directory.like(f'{config.source_basedir.rstrip("/")}%')).all()
    ...
    for local_file in known_files:
      ...
      if local_file.directory not in [target_dir]:
        if os.path.isfile(local_file.full_path):
          print(f'file not sorted yet "{local_file.filename}" is in "{local_file.directory}", should be "{target_dir}"')
          if not os.path.isfile(os.path.join(target_dir, local_file.filename)):
            pathlib.Path(target_dir).mkdir(parents=True, exist_ok=True)
            print(f'##### INFO: moving "{local_file.full_path}" to "{target_dir}"')
            if not config.dry_run:
              shutil.move(os.path.join(local_file.directory, local_file.filename), target_dir)
            ...
            if config.target_crc32:
              print(f'##### INFO: moving "{local_file.full_path}" to "{target_dir}", adding CRC32 to the filename')
              target_filename = local_file.filename[:-len(local_file.filename.split(".")[-1]) - 1] + f'[{local_file.hash_crc}].{local_file.filename.split(".")[-1]}'
              print(f'##### INFO: target filename "{target_filename}"')
              if not config.dry_run:
                shutil.move(os.path.join(local_file.directory, local_file.filename), os.path.join(target_dir, target_filename))
                local_file.directory = target_dir
              ...
              print(f'##### WARNING: "{os.path.join(target_dir, local_file.filename)}" already present')
