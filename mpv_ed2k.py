...
def get_config(config_file=os.path.expanduser('~/.config/weltschmerz/weltschmerz.cfg')):
    ...
    parser = argparse.ArgumentParser(description='Play file matching ed2k in the database.')
...
if __name__ == '__main__':
    ...
    local_file = dbs.session.query(anime.LocalFile).filter(anime.LocalFile.hash_ed2k==ed2k).first()
    if local_file:
