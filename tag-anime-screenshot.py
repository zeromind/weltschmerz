...
def get_config(config_file=os.path.expanduser('~/.config/weltschmerz/weltschmerz.cfg')):
    ...
    parser.add_argument('-d', '--duration', help='file duration',
                        default=None)
    parser.add_argument('-D', '--duration-raw', help='file duration (raw)',
                        default=None)
    ...
    parser.add_argument('--screenshot-type', help='screenshot type', default=1)
...
def tag_screenshot(screenshot, filename, filesize, duration, duration_raw, timepos, timepos_raw, screenshot_type, hash_crc, hash_md5, hash_sha1, hash_ed2k):
...
def tag_screenshot_min(screenshot, filename, filesize, duration, duration_raw, timepos, timepos_raw, screenshot_type):
...
if __name__ == '__main__':
    ...
    try:
        ...
        logging.info(f'found file \'{filename}\' in DB, adding metadata for {config.timepos}...')
        tag_screenshot(
        ...
        logging.info(f'adding title screenshot to DB for \'{target_file}\' at {config.timepos}...')
        tss.filename = target_file
        try:
            anidb_file = dbs.session.query(anime.File).filter(anime.File.hash_ed2k==known_file.hash_ed2k).one()
